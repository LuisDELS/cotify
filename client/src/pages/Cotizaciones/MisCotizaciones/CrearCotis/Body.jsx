...
const Body = ({ filas, moneda }) => {
  ...
  const calcularColumna = (row, columna) => {
    ...
    switch (columna) {
      case 'subtotal':
        return calcularSubtotal(row.precio, row.cantidad);
      case 'iva':
        return calcularIva(row.precio, row.cantidad);
      case 'total':
        return calcularTotal(row.precio, row.cantidad);
      default:
        return 0;
    }
  };
  ...
  function formatNumber(number) {
    ...
    if (isNaN(number) || typeof number !== 'number') {
      return number; // Retornar el valor original si no es un número válido
    }
        ...
        {filas.map((row, index) => (
          !row.eliminada && (
            <View style={styles.tableRow} key={index}>
              <Text style={[styles.tableCellLeft]}>{index+1}</Text>
              <Text style={[styles.tableCellProducto, styles.tableCell]}>{row.nombre}</Text>
              <Text style={[styles.tableCellCant]}>{`${formatNumber(row.cantidad)}`}</Text>
              <Text style={[styles.tableCellPrecio]}>{`${moneda} ${formatNumber(row.precio)}`}</Text>
              <Text style={[styles.tableCellSubtotal]}>{`${moneda} ${formatNumber(calcularSubtotal(row.precio, row.cantidad))}`}</Text>
              <Text style={[styles.tableCellIva]}>{`${moneda} ${formatNumber(calcularIva(row.precio, row.cantidad))}`}</Text>
              <Text style={[styles.tableCellTotal]}>{`${moneda} ${formatNumber(calcularTotal(row.precio, row.cantidad))}`}</Text>
            </View>
          )
        ))}
      </View>
